...
export function Casino() {
  ...
  const { games: ALL, loading } = useCatalog();
  ...
  // only show category tabs that actually have games
  const tabs = useMemo(
    () => [...BASE_TABS, ...CAT_TABS.filter((c) => ALL.some((g) => g.category === c.id))],
    [ALL]
  );
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-soft">
            {loading ? "Loading catalog…" : `${ALL.length} games · certified fair`}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={`Search ${ALL.length} games…`}
